...
This is [Hyperspace's Autoskill](https://x.com/varun_mathur/status/2032224933837684932), built by Varun Mathur's team, extending Andrej Karpathy's autoresearch loop from ML experiments to code generation itself. Skills run inside WebAssembly sandboxes with zero ambient authority: no filesystem, no network, no system calls. Skills invoke other skills recursively, with full lineage tracking. Every mutation knows its parent hash. You can walk the entire evolution tree.
...
| Soulless Swarm | Living System |
|:---|:---|
| Fitness metric decides everything | Purpose filters what gets optimised |
| Convergence on local optima | Divergence maintained by values |
| 795 variants of one thing | Fewer things, done with intent |
...
| Lineage tracks mutations | Lineage tracks meaning |
...
QUESTION: What happens when you build a swarm of autonomous agents with a brain and a body, but no sense of self? 795 of its 1,251 "inventions" were variations of the same thing. Convergence is not evolution.

CONTRAST: Most people celebrate when agents produce more code faster. The data shows something else: without a Soul layer (identity, purpose, values) the swarm converges. It doesn't evolve. Mind and Body without Soul produces optimisation, not innovation.
-->
